...
def parse_transcription(input_data):
    ...
    logger.debug("Parsing transcript: %s lines", len(lines))
    ...
    for line_num, line in enumerate(lines):
        ...
        if not match:
            logger.warning("Could not parse line %s", line_num + 1)
            continue
    ...
    # Generate metadata
    logger.debug("Parsed transcript max end time: %.2f", max_end_time)
